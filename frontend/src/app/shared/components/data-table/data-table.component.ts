    ...
    <div class="overflow-x-auto bg-white rounded-xl shadow-sm border border-slate-100">
      <table class="w-full text-sm text-left text-slate-500">
        <thead class="text-xs text-slate-700 uppercase bg-slate-50 border-b border-slate-100">
          <tr>
            <th *ngFor="let col of columns" class="px-3.5 sm:px-6 py-3 sm:py-4 font-semibold tracking-wider">{{ col.label }}</th>
            <th *ngIf="actionsTemplate" class="px-3.5 sm:px-6 py-3 sm:py-4 font-semibold text-right tracking-wider">Actions</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-slate-100">
          <!-- Loading State -->
          <tr *ngIf="loading" class="bg-white">
            <td [attr.colspan]="columns.length + (actionsTemplate ? 1 : 0)" class="px-3.5 sm:px-6 py-16 sm:py-20 text-center">
          ...
          <tr *ngFor="let row of data" class="bg-white hover:bg-slate-50 transition-colors group">
            <td *ngFor="let col of columns" class="px-3.5 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
            ...
            <td *ngIf="actionsTemplate" class="px-3.5 sm:px-6 py-3 sm:py-4 text-right whitespace-nowrap">
